...
df.to_csv('TSPdata6.txt', index=False)
...
def plot_data(data_type, num_cities):
    ...
    sns.lineplot(data=grouped[grouped['num_cities'] == num_cities], x='max_iterations', y=data_type,
                 hue='max_tabu_size', palette=color_palette)
    plt.xscale('log')
    ...
    plt.savefig(f'analyze_plots6/{data_type}_{num_cities}.png')
    plt.show()
